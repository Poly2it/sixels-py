        ...
        for y in range(0, height, 6):
            ...
            for x in range(width):
                for z in range(min(6, height - y)):
                    value = palette_image.getpixel((x, y + z))
                    if not image.getpixel((x, y + z))[-1] < 255:
                        colors[value][x] |= (1 << z)
